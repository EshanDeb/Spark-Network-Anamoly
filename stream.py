...
def streamDataset(tcp_connection):    # function to stream a dataset
    ...
    print(f"Starting to stream datasets")
    DATASETS = [    # list of files in your dataset to stream
        "train",
        "test"    # uncomment to stream the test dataset
    ]
    for dataset in DATASETS:
        print(f"Starting to stream",dataset)
        file = open("log.txt", "w")
        file.write(dataset)
        file.close()

        streamCSVFile(tcp_connection, f'{dataset}.txt')
        time.sleep(5)
